		...
		switch(path)
		 ...
		 case "/add.mm":
			 ...
			 accountBalance = Double.parseDouble( request.getParameter("accountBalance"));
				 salary = request.getParameter("rdSalary").equalsIgnoreCase("true")?true:false;
			 try {
		 ...
		 case "/fundTransf.mm":
		  ...
		  case "/searchForm.mm":
			...
			case "/sortByName.mm":
				...
				int result = sort ? 1 : -1;
			try {
				inputAccountList = (ArrayList<SavingsAccount>) savingsAccountService.getAllSavingsAccount();
		 		Collections.sort(inputAccountList, new Comparator<SavingsAccount>()
						{
							...
							return result * accountOne.getBankAccount().getAccountHolderName().compareToIgnoreCase(accountTwo.getBankAccount().getAccountHolderName());
							}
